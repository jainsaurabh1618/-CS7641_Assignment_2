...
for prob_size in problem_size_range:
    print(prob_size)
    fit_fn = mlrose_hiive.Knapsack(random.sample(range(10, 500), prob_size), random.sample(range(10, 500), prob_size))
    ds_ops = mlrose_hiive.DiscreteOpt(length=prob_size, fitness_fn=fit_fn, max_val=4)
    # RHC
    print('RHC')
    start = time.time()
    ...
    _, mimic_fitness, mimic_fitness_curve = mlrose_hiive.mimic(ds_ops, max_attempts=mx_attempt, max_iters=mx_iter,
                                                               curve=True, random_state=42, keep_pct=.3)
